...
#        expression( < proper parameters > ,iterator) for iterator in iteratorList
...
def r_squared(y, estimated):
    ...
    """
    # TODO
    numerator = np.sum(np.square(np.subtract(y, estimated)))
    denomiator = np.sum(np.square(np.subtract(y, np.mean(y))))
    return 1 - numerator/denomiator
